...
@section('content')
    <div class="mx-auto max-w-md rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
        <h1 class="text-2xl font-black tracking-tight text-zinc-950">Login</h1>
        <form action="{{ route('login') }}" method="POST" class="mt-6 space-y-4">
            ...
            <button class="w-full rounded-md bg-emerald-600 px-4 py-3 text-sm font-bold text-white hover:bg-emerald-700" type="submit">Login</button>
        </form>
        <p class="mt-5 text-center text-sm text-zinc-600">
            New here?
            <a class="font-bold text-emerald-700 hover:text-emerald-800" href="{{ route('register') }}">Create an account</a>
        </p>
    </div>
@endsection
